...
export const createTask = createAsyncThunk('createTask', async (task) => {
    ...
    return (await fetch(`http://localhost:8080/addtask`, {
        method: 'PUT',
        headers: {
...
export const updateTask = createAsyncThunk('updateTask', async (task) => {
    return (await fetch(`http://localhost:8080/changetask/${task.num}`, {
        method: 'POST',
        headers: {
            'Content-Type': 'application/json'
        },
...
export const deleteTask = createAsyncThunk('deleteTask', async (task) => {
    return (await fetch(`http://localhost:8080/deletetask/${task.num}`, {
        method: `DELETE`
    })).ok;
})
